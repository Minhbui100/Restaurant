...
app.post("/orders", async (req, res) => {
  ...
  try {
    ...
    const billCheck = await pool.query(
      "SELECT 1 FROM Bill WHERE bill_id = $1",
      [id]
    );
    if (billCheck.rowCount === 0) {
      await pool.query(
        "INSERT INTO Bill (bill_id) VALUES ($1)",
        [id] // Default values for total and tax
      );
    }
    await pool.query(
      "INSERT INTO orders (bill_id, name, price, quantity) VALUES ($1, $2, $3, $4)",
